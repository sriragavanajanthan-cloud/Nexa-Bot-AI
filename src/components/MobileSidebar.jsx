...
// IMPORTANT: Define TOOLS here or import from MobileSidebar
// Option 1: Define directly (recommended for simplicity)
const TOOLS = [
  { path: '/chat', label: 'Chat' },
  { path: '/video-studio', label: 'Video' },
  { path: '/image-gen', label: 'Image' },
  { path: '/memory-bank', label: 'Memory' },
  { path: '/ai-detector', label: 'Detector' },
  { path: '/image-editor', label: 'Editor' },
  { path: '/graphing', label: 'Graphs' },
  { path: '/image-amplifier', label: 'Amplify' },
];
...
export default function MobileLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-[#111111] flex">
      {/* Desktop Sidebar */}
      <aside className="fixed left-0 top-0 bottom-0 w-72 bg-[#1a1a1a] border-r border-white/10 flex flex-col">
        ...
        <div className="p-5 border-b border-white/10 bg-white/5">
          <p className="text-xs text-white/50 uppercase">Signed in as</p>
          <p className="text-sm font-medium text-white mt-1 truncate">{userEmail}</p>
        </div>

        <div className="flex-1 py-4">
          <div className="px-4">
            <p className="text-xs text-white/50 uppercase mb-3 px-2">Tools</p>
            <div className="grid grid-cols-2 gap-2">
              {TOOLS.map((tool) => {
                ...
                return (
                  ...
                  >
                    <span className="text-xl">{tool.label === 'Chat' ? '💬' : tool.label === 'Video' ? '🎬' : tool.label === 'Image' ? '🎨' : tool.label === 'Memory' ? '🧠' : tool.label === 'Detector' ? '🛡️' : tool.label === 'Editor' ? '✏️' : tool.label === 'Graphs' ? '📊' : '🔍'}</span>
